            ...
            <table class="table">
                ...
                <tbody>
                    <tr v-for="(item, index) in Temp" :key="index">
                        ...
                        <td>{{ item.nameP }}</td>
                        <td>{{ item.SL }}</td>
                        <td>{{ (item.gia).toLocaleString('en-US', { style: 'currency', currency: 'VND' })  }}</td>
                        <td>{{ (item.SL * item.gia).toLocaleString('en-US', { style: 'currency', currency: 'VND' })  }}</td>
                    </tr>
                </tbody>
                <tfoot>
                    <tr>
                        <td colspan="4" style="text-align: right;"><strong>Tổng cộng:</strong></td>
                        <td>{{ totalAmount.toLocaleString('en-US', { style: 'currency', currency: 'VND' })  }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
...
export default {
    ...
    async created() {
        try {
                ...
                item.revenue.forEach((revenueItem) => {
                    promises.push(
                        new Promise(async (resolve) => {
                            let pd = await axios.get(`http://localhost:3000/api/picture/product/${revenueItem._id}`);
            ...
            // Tính tổng số tiền
            this.totalAmount = this.Temp.reduce((total, item) => total + item.SL * item.gia, 0);
