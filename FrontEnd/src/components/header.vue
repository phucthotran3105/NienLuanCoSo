                    ...
                    <li class="nav-item cart-item ">
                        <router-link class="cart_header" to="/cart">
                            <span class="cart_icon">
                                <i v-if="this.cart.idProduct_item != undefined"
                                    class="fas fa-shopping-cart mr-1 bx bx-cart cartNum"><span>{{
                                        this.cart.idProduct_item.length }}</span></i>
                            </span>
                        </router-link>
                    </li>
                </ul>
...
export default {
    ...
    created() {
        ...
        if (this.idcart != undefined) {
            axios.get(`http://localhost:3000/api/cart/show/${this.idcart}`, {
                headers: {
    ...
    methods: {
        async showData() {
            ...
            if (token != undefined) {
                axios.get(`http://localhost:3000/api/user/show`, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('Token')}`, // Đính kèm Token vào tiêu đề
                    }
                })
